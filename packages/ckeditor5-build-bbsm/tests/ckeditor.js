...
import SalesMessengerEditor from '../src/ckeditor';
...
describe( 'ClassicEditor build', () => {
	...
	describe( 'build', () => {
		it( 'contains plugins', () => {
			expect( SalesMessengerEditor.builtinPlugins ).to.not.be.empty;
		} );

		it( 'contains config', () => {
			expect( SalesMessengerEditor.defaultConfig.toolbar ).to.not.be.empty;
		} );
	} );

	describe( 'create()', () => {
		beforeEach( () => {
			return SalesMessengerEditor.create( editorElement )
				.then( newEditor => {
					editor = newEditor;
				} );
		} );
		...
		it( 'creates an instance which inherits from the ClassicEditor', () => {
			expect( editor ).to.be.instanceof( SalesMessengerEditor );
	...
	describeMemoryUsage( () => {
		testMemoryUsage(
			...
			() => SalesMessengerEditor.create( document.querySelector( '#mem-editor' ) ) );
	} );
} );
